...
/// API service for event-related operations. No auth token.
class EventApiService {
  ...
  /// GET /events-all?festival_id=<festivalId>
  Future<ApiResponse<List<Map<String, dynamic>>>> getEvents(int festivalId) async {
    try {
      ...
      if (data == null) {
        return ApiResponse.error(message: message ?? 'No event data found', statusCode: 200);
      }
      ...
      if (data is List) {
        for (final item in data) {
          try {
            ...
            if (kDebugMode) print('Error parsing event item: $e');
          }
        }
        list = list.where((item) => item.isNotEmpty).toList();
      }

      return ApiResponse.success(data: list, message: message, statusCode: 200);
    } catch (e, stackTrace) {
      final exception = _errorHandler.handleError(e, stackTrace, 'EventApiService.getEvents');
      int? statusCode;
